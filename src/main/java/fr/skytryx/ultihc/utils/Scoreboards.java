...
public class Scoreboards {
	...
	public static void Game(Player player) {
		Bukkit.getScheduler().scheduleSyncRepeatingTask(Bukkit.getPluginManager().getPlugin("UltiHC"),() -> {
		    ...
		    objective.getScore("§3§lTimers").setScore(10);
		    objective.getScore("§aGameTime: §b" + Chronometer.format(Chronometer.get())).setScore(9);
		    if(Integer.parseInt(Settings.get("PvP"))*60-Chronometer.get() > 0) {
		    	objective.getScore("§aPvP: §b"+Chronometer.format(Integer.parseInt(Settings.get("PvP"))*60-Chronometer.get())).setScore(8);
		    }
		    if(Integer.parseInt(Settings.get("FinalHeal"))*60-Chronometer.get() > 0) {
		    	objective.getScore("§aFinalHeal: §b"+Chronometer.format(Integer.parseInt(Settings.get("FinalHeal"))*60-Chronometer.get())).setScore(7);
		    }
		    if(Integer.parseInt(Settings.get("FirstShrink"))*60-Chronometer.get() > 0) {
		    	objective.getScore("§aNext Shrink: §b"+Chronometer.format(Integer.parseInt(Settings.get("FirstShrink"))*60-Chronometer.get())).setScore(6);
		    }
		    ...
		    objective.getScore("§aKills: §b" + KillCount.getKill(player)).setScore(1);
	...
	public static void PostGame(Player player) {
		Bukkit.getScheduler().scheduleSyncRepeatingTask(Bukkit.getPluginManager().getPlugin("UltiHC"),() -> {
		    ...
		    objective.getScore("§aFill: §b" + Fill.fillcount.toString()).setScore(7);
		    objective.getScore("§aKills: §b" + KillCount.getKill(player)).setScore(6);
